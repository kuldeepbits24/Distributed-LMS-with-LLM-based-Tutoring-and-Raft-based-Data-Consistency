...
class LMSServerFollower(lms_pb2_grpc.LMSServicer):
    ...
    def send_heartbeats_as_leader(self):
        ...
        while self.leader_id == self.node_id:
           for follower_ip in ['172.20.10.7:50054']:
...
def serve():
    ...
    lms_pb2_grpc.add_LMSServicer_to_server(LMSServerFollower('node_3'), server)
    server.add_insecure_port('172.20.10.6:50053')  # Updated IP address for Node 3
    logging.info("Raft Follower (Node 3) running on IP 172.20.10.6, port 50053")
    server.start()
